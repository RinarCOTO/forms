...
export async function GET(req: NextRequest, context: { params: Promise<{ id: string }> | { id: string } }) {
  try {
    ...
    const { data: draft, error } = await admin.from('machinery').select('*').eq('id', id).single()

    if (error || !draft) {
      return NextResponse.json({ success: false, message: 'Record not found', error: error?.message }, { status: 404 })
    }
...
export async function PUT(req: NextRequest, context: { params: Promise<{ id: string }> | { id: string } }) {
  try {
    ...
    if (error) return NextResponse.json({ success: false, message: 'Failed to update record', error: error.message }, { status: 500 })
...
export async function DELETE(req: NextRequest, context: { params: Promise<{ id: string }> | { id: string } }) {
  try {
    ...
    if (error) return NextResponse.json({ success: false, message: 'Failed to delete record', error: error.message }, { status: 500 })
