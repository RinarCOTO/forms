  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-2">
        ...
        <FieldCommentBtn fieldKey={fieldKey} comments={comments} onCommentSection={onCommentSection} />
      </div>
      <div
  ...
  return (
    <section className="rpfaas-fill-section shadow-sm p-5">
      <div className="flex items-center justify-between mb-3">
        <h2 className="rpfaas-fill-section-title text-sm uppercase tracking-wide text-muted-foreground">
          {title}
        </h2>
        {fieldKey && comments && onCommentSection && (
          <FieldCommentBtn fieldKey={fieldKey} comments={comments} onCommentSection={onCommentSection} />
        )}
      </div>
      {children}
...
export default function ReviewFormInline({ serverData, comments, onCommentSection }: Props) {
  const {
    ...
    physicalDepreciationPct, depreciationAmount, netUnitCost,
    actualUse, taxStatus, assessmentLevel, assessedValue,
    amountInWords, effectivityOfAssessment, memoranda,
    previousTdNo, previousOwner, previousAv, previousMv, previousArea,
    ...
    <div className="rpfaas-fill max-w-3xl mx-auto py-6">
      ...
      <Section title="General Description">
        <div className="grid grid-cols-2 gap-3">
          ...
          <FieldRow label="Building Age" fieldKey="building_age" value={buildingAge ? (Number(buildingAge) <= 1 ? "New" : `${buildingAge} years`) : ""} {...fp} />
        ...
        {storeys > 0 && (
          <div className="mt-3">
            <p className="text-xs font-semibold text-muted-foreground mb-2 uppercase tracking-wide">Floor Areas</p>
            <div className="grid grid-cols-4 gap-2">
              {Array.from({ length: storeys }, (_, i) => (
                <div key={i} className="border border-border rounded-md px-3 py-1.5 text-sm bg-white text-center">
                  <span className="text-xs text-muted-foreground block">{i + 1}{ordSuffix(i + 1)} Floor</span>
                  {floorAreas[i] ? `${floorAreas[i]} sqm` : <span className="text-muted-foreground">—</span>}
                </div>
              ))}
            </div>
          </div>
      ...
      <Section title="Structural Materials">
        <div className="grid grid-cols-1 gap-3">
          <FieldRow label="Roof Material(s)" fieldKey="roofing_material" value={checkedRoof || "None selected"} {...fp} />
          <FieldRow label="Flooring Material(s) per Floor" fieldKey="flooring_material" value={materialPerFloor(flooringGrid, flooringLabels)} {...fp} />
          <FieldRow label="Wall Material(s) per Floor" fieldKey="wall_material" value={materialPerFloor(wallsGrid, wallLabels)} {...fp} />
        </div>
      </Section>

      {/* 7. Deductions */}
      <Section title="Deductions" fieldKey="selected_deductions" comments={comments} onCommentSection={onCommentSection}>
            ...
            <table className="w-full text-sm border-collapse">
              ...
              <tbody>
                ...
                <tr className="border-t-2 border-border font-semibold bg-muted/30">
                  ...
                  <td className="px-3 py-2 text-center">{deductionsData.reduce((s, d) => s + d.percentage, 0)}%</td>
        ...
        {deductionComments && (
          <div className="mt-2 text-sm text-muted-foreground italic">Comments: {deductionComments}</div>
        )}
      </Section>

      {/* 8. Additional Items */}
      {allAdditional.length > 0 && (
        <Section title="Additional Items" fieldKey="additional_items" comments={comments} onCommentSection={onCommentSection}>
          <div className="overflow-x-auto">
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="bg-muted/50 text-muted-foreground text-xs uppercase tracking-wide">
              ...
              <tbody>
                ...
                <tr className="border-t-2 border-border font-semibold bg-muted/30">
                  <td className="px-3 py-2">TOTAL</td>
                  <td className="px-3 py-2 text-right">{allAdditional.reduce((s, i) => s + i.area, 0)} sqm</td>
                  <td></td>
                  <td className="px-3 py-2 text-right">₱{fmt(allAdditional.reduce((s, i) => s + i.calculatedValue, 0))}</td>
                </tr>
              </tbody>
            </table>
          </div>
      ...
      {/* 9. Depreciation & Market Value */}
      <Section title="Depreciation & Market Value">
        <div className="grid grid-cols-2 gap-3">
          <FieldRow
            label="Physical Depreciation"
            fieldKey="market_value"
            value={physicalDepreciationPct > 0 ? (Number(buildingAge) <= 1 ? "New" : `${physicalDepreciationPct}%`) : ""}
            {...fp}
          />
          <FieldRow
            label="Depreciation Amount"
            fieldKey="market_value"
            value={depreciationAmount > 0 ? `- ₱${fmt(depreciationAmount)}` : ""}
            {...fp}
          />
        </div>
        <div className="mt-3">
          <FieldRow label="Market Value" fieldKey="market_value" value={netUnitCost ? `₱${fmt(netUnitCost)}` : ""} {...fp} />
        </div>
      </Section>
      ...
      <Section title="Property Assessment">
        <div className="grid grid-cols-2 gap-3">
          ...
          <FieldRow label="Tax Status" fieldKey="actual_use" value={taxStatus === "exempt" ? "Exempt" : "Taxable"} {...fp} />
          <FieldRow label="Market Value" fieldKey="market_value" value={netUnitCost ? `₱${fmt(netUnitCost)}` : ""} {...fp} />
        ...
        <div className="mt-3">
          <FieldRow label="Amount in Words" fieldKey="amount_in_words" value={amountInWords ? `${amountInWords} Pesos Only` : ""} {...fp} />
        </div>
      </Section>

      {/* 11. Previous Assessment */}
      <Section title="Previous Assessment">
        <div className="grid grid-cols-2 gap-3">
          <FieldRow label="Prev. TD No." fieldKey="previous_td_no" value={previousTdNo} {...fp} />
          <FieldRow label="Prev. Owner" fieldKey="previous_owner" value={previousOwner?.toUpperCase()} {...fp} />
          <FieldRow label="Prev. AV" fieldKey="previous_av" value={fmtMoney(previousAv)} {...fp} />
          <FieldRow label="Prev. MV" fieldKey="previous_mv" value={fmtMoney(previousMv)} {...fp} />
          <FieldRow label="Prev. Area" fieldKey="previous_area" value={previousArea ? `${previousArea} sqm` : ""} {...fp} />
        </div>
      </Section>
